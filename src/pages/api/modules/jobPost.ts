    ...
    if (Array.isArray(path) && path.length > 1) {   
        ...
        if (param1!== undefined && param1 !== null ){
            ...
            if ( param1 === 'search'){
                ...
                await handleGetPubJobsWithAppls(res,userId, keyword, 
                    orderBy !== '-' ? orderBy : undefined , 
                    ascOrDesc !== '-' ? ascOrDesc : undefined, 
                    isNaN(pageNum) ? 1 : pageNum, isNaN(rowsPerPage) ? 10 : rowsPerPage);
...
async function handleGetPubJobsWithAppls ( 
    ...
    try {
        ...
        let ndata = await getJobPostsWithAppls (userId ?? "", keyword, orderBy, ascOrDesc, p.offset, p.limit);
