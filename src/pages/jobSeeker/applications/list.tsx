...
import { getJobApplications, isErrorUnathorized } from "@/service";
...
export default function List({reloadCount, onEdit} :props) {
    ...
    const refreshResult =  useMemo(() => async (searchString? : string, pageNum? : number ) => {
        setLoading(true);
        let s = await getJobApplications(searchString ?? searchStr, '-', '-', pageNum ?? page , rowsPerPage, (e)=>{
    ...
    <table className="table-auto min-w-full divide-y divide-gray-200">
      ...
      <tr>
        <td colSpan={5} className="text-center">
          <div className="mt-2 text-gray-400 p-2">You Have NOT Applied For Any Job yet</div>
        </td>
      </tr>
      }
      { (searchResult.results.length ?? 0) > 0 &&
